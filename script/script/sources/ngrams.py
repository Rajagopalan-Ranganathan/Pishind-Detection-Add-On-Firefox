...
Code copyright (c) 2015 by Kalle Saari
...
def hillclimb(x, f, neighbors, steps=10000): 
    ...
    for i in range(steps): 
        ...
        if fx2 > fx: 
            x, fx = x2, fx2 
            neighborhood = iter(neighbors(x)) 
    if debugging: print('hillclimb: {} {}'.format(x, int(fx))) 
    return x 
...
def correct(w): 
    ...
    c, edit = max(candidates, key=lambda t: Pedit(t[1]) * Pw(t[0])) 
    return c 
