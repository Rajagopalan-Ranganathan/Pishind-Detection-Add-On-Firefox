...
def _get_screenshot_path(jspath):
    js = _get_json(jspath)
    base = path.dirname(path.dirname(jspath))
    path = path.join(base, 'screenshots', js['siteid'] + '.png')
    return path
...
def prune_link(link):
    ...
    if link.startswith('/'):
        return path.splitext(link[1:])[0]
    if link.startswith('./'):
        return path.splitext(link[2:])[0]
    if link.startswith('../'):
        return path.splitext(link[3:])[0]
    ...
    path = path.splitext(parsed.path)[0]
    return netloc + path
...
if __name__ == '__main__':
    ...
    for jsonpath in jsonpaths:
        ...
        li = _sort_by_count(js, intersection, use_ocr=use_ocr)
